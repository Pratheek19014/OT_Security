...
FILE_STORAGE_PATH = r"C:\\Users\\USER\\Desktop\\MAIT\\CS2025-26\\Scalance S\\UATest\\uploads"
...
server.load_certificate("pki/server/certs/server_cert.pem")
server.load_private_key("pki/server/private/server_key.pem")

server.set_security_IDs(["Username"])

class MyUserManager(UserManager):
    def check_user_token(self, session, user_token):

        username = user_token.UserName
        password = user_token.Password  # already decrypted in SIGN mode

        print("------ LOGIN ATTEMPT ------")
        print("Username:", username)
        print("Password (raw bytes):", password)

        try:
            decoded_password = password.decode().strip()
        except:
            decoded_password = ""

        print("Decoded password:", decoded_password)

        if username == "admin" and decoded_password == "1234":
...
server.user_manager = MyUserManager(server)

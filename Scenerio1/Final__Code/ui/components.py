...
def render_connection_status(connected, error_message=None, pending=False):
    """Render OPC UA connection status indicator"""
    if pending and connected is None:
        st.warning("Connecting to OPC UA Server...")
    elif connected:
        st.success("Connected to the Server")
    else:
        if error_message:
            st.error(f"Server security issue: {error_message}")
        else:
